...
export const downloadPDF = async (transcript: Transcript) => {
  ...
  const primaryColor: [number, number, number] = [40, 75, 99]; // #284b63
  const secondaryColor: [number, number, number] = [172, 118, 133]; // #ac7685
...
export const downloadAnalyticsReport = async (stats: UsageMetrics) => {
    try {
        ...
        doc.text("Generated by LoquiHQ", margin, yPos);
...
export const downloadDOCX = async (transcript: Transcript) => {
  ...
  try {
    ...
    const createHeading = (text: string, level: typeof HeadingLevel.HEADING_1 | typeof HeadingLevel.HEADING_2 | typeof HeadingLevel.HEADING_3 = HeadingLevel.HEADING_2) => {
