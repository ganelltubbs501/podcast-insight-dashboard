...
export async function analyzeWithGemini(payload: {
  ...
  if (s) {
    if (s.accuracyLevel === "High") systemInstruction += " Perform a detailed analysis, ensuring no nuance is missed.";
    else if (s.accuracyLevel === "Maximum") systemInstruction += " Perform an exhaustive, critically detailed analysis. Prioritize depth over brevity.";

    if (s.toneFilter && s.toneFilter !== "Auto") systemInstruction += ` Analyze the content specifically focusing on ${s.toneFilter} language patterns and nuances.`;
    if (s.language && s.language !== "Auto") systemInstruction += ` The content language is ${s.language}. Ensure all output is in this language unless specified otherwise.`;
    if (s.sensitiveContentFilter) systemInstruction += " Please flag and treat sensitive topics with care, using neutral language where appropriate.";
    if (s.dialectContext) systemInstruction += ` Note: The speakers may use ${s.dialectContext} dialect or idioms. Interpret accordingly.`;
  }

  let textPrompt = `Analyze the following podcast content (text, audio, or image of notes) and generate a comprehensive set of insights, content, and analytics.`;
  ...
  properties: {
    ...
    socialContent: {
      ...
      properties: {
        linkedinPost: { type: Type.STRING, description: "Professional LinkedIn post (150-300 words) with hashtags and engagement question." },
        twitterThread: {
          type: Type.ARRAY,
          description: "A thread of 5-10 tweets, numbered.",
          items: { type: Type.STRING }
        },
        tiktokScript: { type: Type.STRING, description: "A 30-second script for a Reel/TikTok video with visual cues and hooks." },
        youtubeDescription: { type: Type.STRING, description: "SEO-optimized YouTube Shorts description (100-150 chars)." },
        emailNewsletter: {
          type: Type.OBJECT,
        ...
        mediumArticle: { type: Type.STRING, description: "Detailed article draft formatted in Markdown (H2, H3) ~1000 words." },
        newsletterTeaser: {
          type: Type.OBJECT,
    ...
    showNotes: {
      type: Type.STRING,
      description: "Formatted show notes with timestamps, key topics discussed, and mentioned resources."
    },
    faq: {
      type: Type.ARRAY,
  ...
  required: ["keyTakeaways", "quotes", "socialContent", "blogPost", "showNotes", "faq", "seo", "speakers", "sentiment", "socialClips"]
}
